...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "UI/UX design for any niche, business sites, stores, apps & more. Wireframes, user flow mapping, and interfaces built around real users."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Good UI/UX design is invisible when it works the user completes their task without thinking about the interface at all. It only becomes visible when it's done wrong."
