...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Webflow Development Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Webflow development for design-forward brands, custom animations, CMS-driven content, and clean, hand-off-ready builds by an in-house team."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Webflow gives you design control most platforms can't match, but that freedom is exactly how a build turns into a mess of unstructured sections if it isn't planned properly. We architect the CMS and design system first."
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Webflow Development</span>
        </div>
      </nav>
