...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Custom / Next.js Development Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Custom Next.js development for businesses that have outgrown template platforms, full control, real performance, and scalable architecture."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Most projects don't need custom development; they need a well-built platform site. Custom is the right call when you're hitting a wall a template genuinely can't solve, not because custom sounds more impressive."
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Custom / Next.js Development</span>
        </div>
      </nav>
