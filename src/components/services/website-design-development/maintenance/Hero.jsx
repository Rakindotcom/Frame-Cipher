...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Website Maintenance Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Website maintenance for any niche business sites, stores, blogs & more. Security updates, backups, and monitoring by an in-house team."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Most site problems don't happen suddenly, they build up quietly through skipped updates, growing plugin conflicts, or unmonitored uptime, until something finally breaks in front of a customer. Maintenance is what catches that before it happens."
