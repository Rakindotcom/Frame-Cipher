...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Magento Development Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Magento development for large catalogs and complex ecommerce, multi-store setups, B2B pricing, and custom integrations built by an in-house team."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Magento's overhead only pays off past a certain scale. Below that, it's more infrastructure than a business needs. We'll tell you directly if your catalog doesn't need it yet."
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Magento Development</span>
        </div>
      </nav>
