...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "Motion Graphics & Animation for concepts a camera can't film explainers, data visualization, logo stings, and animated ad content."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Motion Graphics & Animation example showing an explainer video style frame"
