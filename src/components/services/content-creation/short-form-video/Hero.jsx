...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Short-Form Video Production Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Reels, Shorts, and TikTok production built for how vertical video actually gets discovered fast hooks, native pacing, platform-ready delivery."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The scroll doesn't wait for a slow start. If the first second doesn't earn the second one, everything well-crafted after it never gets seen."
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Short-Form Video Production</span>
        </div>
      </nav>
