...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "The actual visuals a brand runs on video, photography, motion graphics, and design, produced by one in-house creative team instead of a different freelancer for every format. See our portfolio and get a free consultation on what your brand actually needs made."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Get a Free Production Consultation"
