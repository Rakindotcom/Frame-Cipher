...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "Logo Design built to work at every size and in every context a mark actually gets used, not just as one polished file that looks good once."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Logo Design example showing a mark at different sizes and color variations"
