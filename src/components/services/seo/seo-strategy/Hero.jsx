...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "SEO strategy and consulting for teams that need direction, not just execution, roadmaps, audits, and expert guidance from an in-house team."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The best SEO strategy is the one your team can actually execute, not the most theoretically complete one. A roadmap that assumes resources you don't have isn't a strategy, it's a wish list."
