...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "MVP development for founders and businesses alike, lean, functional builds that validate demand before you commit to full-scale development."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The point of an MVP isn't to build less software. It's to learn whether you should build more of it, and that only works if what you ship actually functions well enough to trust the feedback."
