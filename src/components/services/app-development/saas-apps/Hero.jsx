...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "SaaS app development built for subscriptions and scale, multi-tenant architecture, billing integration, and infrastructure that grows with you."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The gap between a SaaS demo and a SaaS product that survives its first thousand paying customers is almost entirely architecture. Everything else is featured."
