...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "iOS app development in Swift, polished performance, App Store deployment, and apps built to Apple's design and review standards."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "A rejected submission near a launch date is one of the most avoidable delays in app development, and almost always traces back to a decision made months earlier, not a mistake made at the review stage."
