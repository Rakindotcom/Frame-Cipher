...
export default function Hero({ service }) {
  ...
  const subtitle = service?.shortDesc || service?.metaDescription || "Enterprise app development for complex workflows, legacy system integration, SSO, compliance, and applications built around how your org actually runs."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The hardest part of enterprise development is rarely the code. It's building something that works within an organization's existing systems, security requirements, and approval structure, without becoming the thing everyone routes around."
