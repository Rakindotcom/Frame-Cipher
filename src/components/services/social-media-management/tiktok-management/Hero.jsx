...
export default function Hero({ service }) {
  const title = service?.h1 || "TikTok Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "TikTok Management covers content strategy, posting, community engagement, and organic growth, built around how the platform actually surfaces content."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "TikTok Management example showing organic content and profile growth"
