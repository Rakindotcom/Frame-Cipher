...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Community Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Community management across every platform timely, on-brand responses to comments, messages, and reviews, monitored like they matter."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "work of posting. A single well-handled negative comment, answered publicly and professionally, often does more for trust than the content that prompted it in the first place."
