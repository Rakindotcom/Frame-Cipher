...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Facebook Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Facebook Page management built for how the platform works today community, trust signals, and local discovery, not just organic reach."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Facebook stopped being primarily a reach platform for businesses years ago. Treating it like one is how a Page ends up posting into a void and calling it a strategy."
