...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Instagram Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Instagram management built around what the algorithm actually rewards now, Reels, genuine engagement, and a visual identity worth following."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Instagram stopped being a photo-sharing app with ads a while ago. It's a short-form video platform with a legacy photo feature attached, and strategy that hasn't caught up to that is leaving reach on the table."
