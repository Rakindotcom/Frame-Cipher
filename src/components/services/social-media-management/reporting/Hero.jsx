...
export default function Hero({ service }) {
  const title = service?.h1 || "Monthly Reporting & Analytics Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Social media reporting that explains what happened and why engagement, growth, and content performance with context, not raw exports."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "A number without context isn't information, it's just a number. The job of a report is answering 'so what,' not just 'what happened.'"
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Monthly Reporting & Analytics</span>
        </div>
      </nav>
