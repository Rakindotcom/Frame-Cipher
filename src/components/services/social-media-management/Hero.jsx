...
export default function Hero({ service }) {
  const title = service?.h1 || "Social Media Management Services in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "A social presence that actually looks after itself, content planned, posted, and engaged consistently across every platform that matters, handled by one in-house team. Get a free social audit and see what's actually working right now."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "See What Your Profiles Say About You"
