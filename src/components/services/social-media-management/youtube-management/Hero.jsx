...
export default function Hero({ service }) {
  const title = service?.h1 || "YouTube Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "YouTube Management covering video SEO, thumbnails, upload strategy, and channel growth, built for search that keeps working long after upload."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "YouTube Management example showing channel content and thumbnail strategy"
