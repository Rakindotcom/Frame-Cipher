...
export default function Hero({ service }) {
  const title = service?.h1 || "Remarketing Services in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Remarketing services built on proper audience segmentation, frequency control, and first-party tracking, not one generic ad shown on a loop."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Remarketing services example showing audience segments across ad platforms"
