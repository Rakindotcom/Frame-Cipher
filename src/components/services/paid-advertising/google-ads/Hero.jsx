...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Google Ads Management Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Google Ads management built around conversions, not clicks, Search, Shopping, and Performance Max campaigns managed by an in-house team."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Google Ads doesn't work for us"
