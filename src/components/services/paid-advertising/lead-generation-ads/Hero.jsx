...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Lead Generation Ads Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Lead generation ads across Google, Meta, and LinkedIn, campaigns built around inquiry quality and fast follow-up, not just form-fill counts."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "The form submission isn't the finish line, it's the start of a race against how fast a competitor might follow up. A great ad feeding a slow response process is solving only half the problem."
