...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Content Strategy Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Content strategy that decides what to write and why before anything gets drafted, audits, calendars, and priorities built around real goals."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "A full content calendar and a real content strategy aren't the same thing. One is a production schedule. The other is a reason those specific pieces, in that specific order, actually add up to something."
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Content Strategy</span>
        </div>
      </nav>
