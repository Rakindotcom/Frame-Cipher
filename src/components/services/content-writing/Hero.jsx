...
export default function Hero({ service }) {
  const title = service?.h1 || "Content Writing Services in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Writing built to do something specific rank, sell, explain, or convert; not generic filler dressed up as content. Get a free content sample and see the difference before committing to anything."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "See What Your Content Could Do"
