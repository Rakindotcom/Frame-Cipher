...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Case Study Writing Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Case study writing built on real specifics, actual numbers, actual quotes, actual process, not a vague testimonial dressed up as proof."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Great service, highly recommended"
