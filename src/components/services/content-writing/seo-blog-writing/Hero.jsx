...
export default function Hero({ service }) {
  const title = service?.h1 || "Best SEO & Blog Writing Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "SEO blog writing built to rank and actually get read real keyword research, genuine expertise, and content structured to hold attention."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "A blog post ranking for a keyword nobody clicks, or getting clicks and immediately losing the reader, both are the same failure wearing a different costume. The content didn't actually do its job either way."
