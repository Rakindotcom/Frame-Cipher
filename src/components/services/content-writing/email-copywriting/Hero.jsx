...
export default function Hero({ service }) {
  const title = service?.h1 || "Best Email Copywriting Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Email copywriting built to actually get opened and read, welcome sequences, nurture campaigns, and promotional emails that earn attention."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "An email's first job isn't to persuade, it's to survive being deleted. Nothing else in the copy matters if the subject line doesn't earn the opening first."
