...
export default function Hero({ service }) {
  const title = service?.h1 || "Landing Page Copywriting Service in Bangladesh"
  const subtitle = service?.shortDesc || service?.metaDescription || "Landing Page Copywriting built around one goal per page, message-matched to the ad that sent the click, structured to convert, not just read well."
  const isPillar = service?.pageType === 'Pillar Service'
  const pillarParent = service?.pillarParent
  const quote = service?.quote || "Landing Page Copywriting example showing conversion-focused page structure"
    ...
    <div className="bg-frame-bg text-frame-fg">
      ...
      <nav aria-label="Breadcrumb" className="border-b border-frame-border/60 bg-frame-bg/80 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-frame-muted-fg md:px-8">
        <div className="mx-auto flex max-w-[95vw] items-center gap-2 overflow-x-auto">
          ...
          <span>/</span>
          <span className="text-frame-accent">Landing Page Copywriting</span>
        </div>
      </nav>
